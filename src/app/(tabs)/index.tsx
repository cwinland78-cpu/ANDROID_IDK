...
import { View, Text, Pressable, Image, ScrollView, Linking, ActivityIndicator, Share as RNShare, Platform } from 'react-native';
...
export default function HomeScreen() {
  ...
  const openInMaps = useCallback((place: Place) => {
    ...
    const shouldUseGoogleMaps = Platform.OS === 'android';
    ...
    const locationSuffix = userLocation
      ? `&center=${userLocation.latitude},${userLocation.longitude}`
      : '';
    const appleMapsUrl = userLocation
      ? `https://maps.apple.com/?q=${encodeURIComponent(searchQuery)}&near=${userLocation.latitude},${userLocation.longitude}`
      : `https://maps.apple.com/?q=${encodeURIComponent(searchQuery)}`;
    const googleMapsUrl = `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(searchQuery)}${locationSuffix}`;

    const primaryUrl = shouldUseGoogleMaps ? googleMapsUrl : appleMapsUrl;
    const fallbackUrl = shouldUseGoogleMaps ? appleMapsUrl : googleMapsUrl;

    Linking.openURL(primaryUrl).catch(() => {
      // Fallback to the other maps provider
